...
def Run(prefix, **kwargs):
    ...
    RotationAngle_common.WhichScint = ROOT.kScintDenoisedEnergy
    ROOT.EXOScintEventSummary.fgWhichCharge = ROOT.EXOScintEventSummary.kDenoised
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        print 'python ComputeRotationAngle_WeightedScint.py <run_number>'
        sys.exit()
    import ROOT
    ROOT.gROOT.SetBatch()
    ...
    result = Run('RotationAngleDenoised', EventTree=EventTree, ControlRecordList=ControlRecordList)
    print result
